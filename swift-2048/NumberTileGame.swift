...
class NumberTileGameViewController : UIViewController, GameModelProtocol {
  ...
  func reset() {
    ...
    @objc func buttonBeforeClicked(sender:UIButton)
    {
        ...
        alertView.addButton(withTitle: "記錄這神聖的一刻吧～")
        alertView.show()
...
//      let viewHeight = views[order].bounds.size.height
    ...
    //設置按鈕位置和大小
    buttonBefore.frame = CGRect(x:10, y:200, width:100, height:30)
    //設置按鈕文字
    buttonBefore.setTitle("一鍵加速", for:.normal)
    buttonBefore.backgroundColor = UIColor.brown
    ...
    buttonBack.frame = CGRect(x:10, y:250, width:100, height:30)
    buttonBack.setTitle("回前頁", for: .normal)
    buttonBack.backgroundColor = UIColor.brown
  ...
  func followUp() {
    ...
    if m.userHasLost() {
      // TODO: 跳出alert
      NSLog("You lost...")
        let alertController = UIAlertController(title: "Title", message: "喔喔輸了～", preferredStyle: .alert)
        ...
        let cancelAction = UIAlertAction(title: "登入成績", style: UIAlertActionStyle.cancel) {
            UIAlertAction in
